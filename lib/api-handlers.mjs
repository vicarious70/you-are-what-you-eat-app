...
// GET  /api/beverages   -> list   |   POST -> log a drink, return { beverage, analysis, dna }
export const handleBeverages = withUser(async (req, res, { userId, engine }) => {
  if (req.method === "GET") {
    return sendJson(res, 200, { beverages: await engine.store.listBeverages(userId) });
  }
  if (req.method === "POST") {
    const body = await readJsonBody(req);
    return sendJson(res, 200, await engine.logBeverage({ ...body, userId }));
  }
  return sendJson(res, 405, { error: "Method not allowed." });
});
